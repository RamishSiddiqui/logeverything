...
class PaymentService:
    ...
    async def process_payment_with_retry(self, order: Order, max_retries: int = 3) -> PaymentResult:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                self.log.info(
                    f"💳 Processing payment for order {order.id} (attempt {attempt}/{max_retries})"
                )
    ...
    async def _make_payment_api_call(self, order: Order) -> PaymentResult:
        ...
        return PaymentResult(
            success=True, transaction_id=transaction_id, error_message=None, processing_fee=fee
        )
    ...
    def _log_successful_transaction(self, result: PaymentResult) -> None:
        ...
        self.log.info(
            f"💰 Transaction {result.transaction_id} completed (fee: ${result.processing_fee:.2f})"
        )
...
class OrderProcessingService:
    ...
    def _generate_workflow_result(
        self, order: Order, payment: PaymentResult, shipping: List[Dict]
    ) -> Dict[str, Any]:
    ...
    @log(using="order_processor")
    async def process_multiple_orders_concurrently(
        self, orders_batch: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        async with self.log.context(f"BatchProcess-{batch_id}"):
            ...
            self.log.info(
                f"📊 Batch {batch_id} completed: {success_count} success, {failure_count} failures"
            )
            return [
                r if isinstance(r, dict) else {"status": "error", "reason": str(r)} for r in results
            ]
